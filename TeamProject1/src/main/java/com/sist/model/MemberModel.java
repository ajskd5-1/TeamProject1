...
/* 비밀번호 수정 */
	@RequestMapping("member/pwd_update.do")
	public String member_pwd_update(HttpServletRequest request, HttpServletResponse response)
	{
		HttpSession session=request.getSession();
		String id=(String)session.getAttribute("id");
		request.setAttribute(id, id);
		request.setAttribute("mypage_jsp", "../member/pwd_update.jsp");
		request.setAttribute("main_jsp", "../mypage/mypage.jsp");
		return "../main/main.jsp";
	}
	@RequestMapping("member/pwd_check_ok.do")
	public String member_pwd_check(HttpServletRequest request, HttpServletResponse response)
	{
		String pwd=request.getParameter("pwd");
		String id=request.getParameter("id");
		String bCheck=MemberDAO.memberPwdInfo(id, pwd);
		request.setAttribute("bCheck", bCheck);
		return "../member/pwd_check_ok.jsp";
	}
	@RequestMapping("member/pwd_update_ok.do")
	public String member_pwd_update_ok(HttpServletRequest request, HttpServletResponse response)
	{
		try 
		{
			request.setCharacterEncoding("UTF-8");
...
/* 회원탈퇴 */
	...
	public String member_join_delete_ok(HttpServletRequest request, HttpServletResponse response)
	{
		String pwd=request.getParameter("pwd");
		String id=request.getParameter("id");
		String bCheck=MemberDAO.memberDelete(id, pwd);
		HttpSession session=request.getSession();
		if(bCheck=="true")
		{
			session.invalidate();
